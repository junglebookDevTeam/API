...
<ul>
  ...
  <ul>
    <li><code>goodsNo</code>(int 10): 상품번호</li>
	<li><code>open</code>(int 1): 진열여부 (* open 값이 "0" 인 경우(미진열 상품) 이하 상품정보값은 표시 하지 않습니다.)</li>
    <li><code>goodsNm</code>(varchar 255): 상품명</li>
    <li><code>categoryCode</code>(varchar 50): 카테고리코드(SEPERATOR "|")</li>
    <li><code>categoryNm</code>(varchar 100): 카테고리명(SEPERATOR "|")</li>
    <li><code>origin</code>(varchar 50): 원산지</li>
    <li><code>maker</code>(varchar 50): 제조사</li>
    <li><code>brand</code>(varchar 20): 브렌드</li>
    <li><code>goodsPrice</code>(int 10): 상품판매가(파트너사의 매입가)</li>
    <li><code>suggestionSalesPrice</code>(int 10): 파트너사의 권장 판매가</li>
		<ul>
	...
	<li><code>sspProfitRate</code>(int 10): 권장 판매가 마진율(%), (suggestionSalesPrice의 10의 자리 올림 처리로 계약 마진율 보다 같거나 높을 수 있습니다.)</li>
	<li><code>suggestionRetailPrice</code>(int 10): 권장 소비자가</li>
    <li><code>goodsDetail</code>(mediumtext): 상품상세정보</li>
	<li><code>goodsRepImage</code>(varchar 255): 상품대표 이미지</li>
    <li><code>goodsImage</code>(array): 상품 추가 이미지</li>
    <li><code>goodsStock</code>(int 10): 상품재고수</li>
    <li><code>runout</code>(int 1): 품절여부</li>
	<li><code>barcode</code>(varchar 50): 상품 바코드 (바코드 시작 값이 "A" 인 경우 정글북에서 임의 생성한 바코드 입니다. 경우에 따라 상품의 실제 바코드와 다를 수 있습니다.)</li>
    <li><code>EAD</code>(date): 재입고일</li>
    <li><code>inPackageEA</code>(int 10): 패키지 입수량</li>
	<li><code>tags</code>(varchar 500): 상품 특이사항(SEPERATOR ",")</li>
  </ul>
</ul>
